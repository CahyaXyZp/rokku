...
4. Confirmed for this branch: multi-account is back in scope (token-based auth makes it straightforward), presence only while reading, incognito reuses Rokku's existing global + per-extension mechanism, new 18+ suppression toggle (flag sourced from the extension), custom cover art without a third-party proxy dependency.
